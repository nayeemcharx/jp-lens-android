...
- **MODE_SENTENCE_DICT (2)** — same sentence boxes/popup as LLM mode but **fully offline, no LLM**. Word-by-word = Kuromoji morphemes glossed against the bundled JMdict (`JmDict.lookupWord`); Translation = Google Translate of the full sentence (`Translator`); **no standalone Kanji section** (unlike LLM mode) and no Notes section. Each word row has a **▸ chevron** that lazily expands a full JMdict detail panel below it — all senses with part-of-speech, misc/field/dialect tags, notes, cross-refs, loanword origin, alternate writings/readings, **plus a Kanji subsection for the kanji in that word** (KANJIDIC2 meaning + JLPT via `JmDict.lookupKanji`/`kanjiInfoForWord`) — via `JmDict.lookupWordDetail` + `OverlayService.renderWordDetail`; tap again to collapse (expand/collapse does **not** reposition the popup — it stays anchored top-left and grows). The "+" Anki card here uses **front = the word (kanji form, no kana)** and **back = full JMdict detail + the word's kanji + the source sentence + its translation, rendered as HTML** (`buildAnkiBackHtml`), not the summary. Requires `assets/jmdict.db` to have been built first (see Build / run).
...
| File | Role |
|---|---|
| `MainActivity.kt` | Compose UI: a Bedrock-token field + just **Start** and **Stop** (mode is no longer chosen here). `startCapture()` requests notification/overlay permissions inline (opens the overlay-settings screen if needed) then launches the `MediaProjectionManager` consent dialog; it resumes the **last-used mode** (`PREF_LAST_MODE`, default morpheme). Mode is switched live afterwards via the floating button's long-press radial menu (see `OverlayService`). Launches `OverlayService` as a foreground service after the consent dialog. Also handles the AnkiDroid runtime-permission prompt: when launched with extra `EXTRA_REQUEST_ANKI_PERMISSION`, a `LaunchedEffect` fires `RequestPermission` for `AnkiDroidHelper.PERMISSION` (the overlay Service can't request dangerous permissions itself). |
...
- **Live mode switching**: the floating button's touch listener adds a long-press (`MENU_HOLD_MS`, ~0.5s; a drag past `touchSlop` cancels it) that opens `showRadialMenu` — a full-screen dimmed scrim with circular option buttons (the 3 modes + Stop) arranged around the button. Selecting a mode calls `setMode`, which swaps the `mode` field, clears stale boxes, warms `JmDict` for dict mode, and persists `PREF_LAST_MODE` — no new projection consent needed (the `MediaProjection` is already granted). A drag (movement past `touchSlop`) cancels the pending long-press so dragging the button still works.
